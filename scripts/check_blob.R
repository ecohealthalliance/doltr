...
setMethod("st_read", signature(dsn = "DoltLocalConnection", layer = "character"),
          function(dsn = NULL,
                   layer = NULL,
                   query = NULL,
                   EWKB = FALSE,
            ...
            if (!is.null(layer)) {
              if (!is.null(query)) {
                warning(
                  "You provided both `layer` and `query` arguments,",
                  " will only use `layer`.",
                  call. = FALSE
                )
              }
              ...
              if (inherits(dsn, "PostgreSQLConnection")) {
                tbl <-
                  filter_warning(dbReadTable(dsn, layer),
                                 "unrecognized PostgreSQL field type geometry")
              } else {
