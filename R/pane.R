  ...
  if (!is.null(observer) && interactive() && dbIsValid(conn)) {
    ...
    observer$connectionOpened(
      ...
      listObjects = function(schema = NULL, versioning = NULL) {
        if (is.null(schema)) {
          HEAD <- paste0("HEAD: ", dolt_head(conn, show_sql = FALSE))
          statusline <- dolt_statusline(dolt_status(conn = conn, collect = TRUE))
          tbls <- dbListTables(conn)
          out <- data.frame(
            name = c(HEAD, statusline, tbls, "dolt_system_tables", "information_schema"),
            type = c("info", rep("info", length(statusline)), rep("table", length(tbls)), "schema", "schema"),
